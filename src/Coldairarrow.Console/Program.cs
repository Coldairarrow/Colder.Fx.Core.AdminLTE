...
namespace Coldairarrow.Console1
{
    class Program
    {
        ...
        static async Task RunServerAsync()
        {
            ...
            var theServer = await SocketBuilderFactory.GetTcpSocketServerBuilder(6001)
                ...
                .OnRecieve((server, connection, bytes) =>
                {
                    ...
                    server.GetAllConnections().ForEach(x => x.Send(bytes));
            ...
            var theClient1 = await SocketBuilderFactory.GetTcpSocketClientBuilder("127.0.0.1", 6001)
                .SetLengthFieldEncoder(2)
                .SetLengthFieldDecoder(ushort.MaxValue, 0, 2, 0, 2)
            ...
            var theClient2 = await SocketBuilderFactory.GetTcpSocketClientBuilder("127.0.0.1", 6001)
                .SetLengthFieldEncoder(2)
                .SetLengthFieldDecoder(ushort.MaxValue, 0, 2, 0, 2)
                .OnClientStarted(client =>
                {
            ...
            await theClient1.Send("小明");
            await theClient2.Send("小明");
        ...
        {
            var theServer = await SocketBuilderFactory.GetWebSocketServerBuilder(6001)
                .OnConnectionClose((server, connection) =>
                {
                    Console.WriteLine($"连接关闭,连接名[{connection.ConnectionName}]");
                })
                ...
                {
                    Console.WriteLine($"服务端:收到数据{msg}");
                    server.GetAllConnections().ForEach(x => x.Send(msg));
                })
                .OnSend((server, connection, bytes) =>
                {
                    //Console.WriteLine($"向连接名[{connection.ConnectionName}]发送16进制数据:{bytes.To0XString().ToUpper()}");
            ...
            var theClient1 = await SocketBuilderFactory.GetWebSocketClientBuilder("127.0.0.1", 6001,"/")
                .OnClientStarted(client =>
                {
                    Console.WriteLine($"客户端启动");
                })
                ...
                .OnRecieve((client, msgStr) =>
                {
                    Console.WriteLine($"客户端:收到数据{msgStr}");
                })
                .OnSend((client, bytes) =>
                {
                                //Console.WriteLine($"客户端:发送16进制数据{bytes.To0XString().ToUpper()}");
                ...
                .BuildAsync();
            //Thread.Sleep(1000);
            await theClient1.Send("你好");
            Thread.Sleep(1000);
            await theClient1.Send("你好22");

            Console.ReadLine();
        }

        static async Task RunUdpSocketServer()
        {
            var theServer = await SocketBuilderFactory.GetUdpSocketBuilder(6001)
                .OnClose(server =>
                {
                ...
                {
                    Console.WriteLine($"服务端:收到来自[{point.ToString()}]数据{bytes.ToString(Encoding.UTF8)}");
                })
                .OnSend((server, point, bytes) =>
                {
                    Console.WriteLine($"服务端发送数据:目标[{point.ToString()}]数据{bytes.ToString(Encoding.UTF8)}");
                })
                .OnStarted(server =>
                {
                    Console.WriteLine($"客户端启动");
            ...
            await theClient.Send("小明", "127.0.0.1:6001".ToIPEndPoint());
            await Task.Delay(1000);
            await theClient.Send("小明111", "127.0.0.1:6001".ToIPEndPoint());
